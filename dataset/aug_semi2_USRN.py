...
class SemiDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        img = Image.open(self.root + id.split(' ')[0]).convert('RGB')
        mask = Image.fromarray(np.array(Image.open(self.root + id.split(' ')[1])))
        ...
        cutmix_box = obtain_cutmix_box(img_s.size[0], p=1.0)        # all cutmix
